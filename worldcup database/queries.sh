...
echo "$($PSQL "SELECT DISTINCT t.name FROM games g JOIN teams t ON t.team_id IN (g.winner_id, g.opponent_id) WHERE g.year = 2014 ORDER BY t.name")"
...
echo "$($PSQL "SELECT DISTINCT g.year, t.name FROM games g JOIN teams t ON g.winner_id = t.team_id WHERE g.round = 'Final' ")"
